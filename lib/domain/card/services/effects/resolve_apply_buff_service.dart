...
part 'resolve_apply_buff_service.g.dart';

@riverpod
ResolveApplyBuffService resolveApplyBuffService(Ref ref) {
  return ResolveApplyBuffService();
}

class ResolveApplyBuffService {
  ApplyActionResult execute({
    required GameState state,
    required CardEffectApplyBuff effect,
    ...
    final newPlayerState = targetPlayer.applyBuffState(
      effect.buff,
      effect.stacks,
    );
    ...
    final step = GameStepEvent.buffApplied(
      targetPlayerId: newPlayerState.id,
      buff: effect.buff,
      stack: effect.stacks,
      totalStack: newPlayerState.getBuffStack(effect.buff),
    );
